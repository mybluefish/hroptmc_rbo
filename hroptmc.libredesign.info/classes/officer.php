...
class Officer{
	...
	const CONST_NOT_A_MEMBER_VALIDDATE = "validDate";
	const CONST_NOT_A_MEMBER_EXPIREDATE = "expireDate";
	...
	public function isOfficerRole($roleName){
		return (($roleName != Officer::CONST_NOT_A_MEMBER_VALIDDATE) && ($roleName != Officer::CONST_NOT_A_MEMBER_EXPIREDATE));
	}

	public function addMyselfIntoRecords(){
		global $officerTableKeys;
		...
		foreach ($officerTableKeys as $key => $value) {
			...
			if($key != (count($officerTableKeys) - 1)){
				$tableFieldList .= ",";
			}
		}

		$sqlSentence = "INSERT INTO `".Officer::CONST_TABLE_NAME."` (".$tableFieldList.") VALUES (".$valueList.");";
