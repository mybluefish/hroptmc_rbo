...
class RegularMeeting{
	...
	public function getSingleRole($roleClass, $roleIndex, $noOfAgendasInOneScreen, $idIndex){
		if($this->date->isNormalMeeting() || $roleIndex == RegularMeeting::CONST_ROLE_INDEX_PRESIDENT){
			...
			return "<td rowspan='".(count($this->agendaRoleNames) - 1)."' id='meetingNotRegular".$this->date->toString()."' class='meetingNotRegular' onMouseOver=\"changeColorMeetingNotRegular('meetingNotRegular".$this->date->toString()."', true)\" onMouseOut=\"changeColorMeetingNotRegular('meetingNotRegular".$this->date->toString()."', false)\">".$this->date->getExceptionalReason()."</td>";
		}
	}
	...
	private function printNames($roleName, $regName){
		...
		if($regName == "" || $regName == null || $regName == "NULL"){
			if((!RoleNames::isPresident($roleName)) && (!$this->date->isInThePast()))
				$namesToBePrinted = "<a href='javascript:void(0)' onClick=\"regRole('".$this->date->toString()."','".$roleName."',event)\">预约</a>";
			else
				$namesToBePrinted = "-";
		} else {
	...
	private function insertPresidentToAgendaTable(){
		...
		$presidentId = Officer::getOfficerId($this->date->toString(), Officer::CONST_FIELD_PRESIDENT);
	...
	public function echoClosed(){
		return "(<span id='meetingclosed'>关闭</span>)";
	}

	public function echoPassed(){
		if(isset($_SESSION["USERNAME"])){
			return  "(<span id='meetingpassed'><a id='editMeetingLink' href='javascript:void(0)' onClick='meetingDateChangeOfAgendaManage(\"".$this->date->toString()."_close\")'>过期</a></span>)
			...
			return  "(<span id='meetingpassed'>过期</span>)";
		}
	}

	public function echoOpen(){
		if(isset($_SESSION["USERNAME"])){
			return  "(<span id='meetingopen'><a id='editMeetingLink' href='javascript:void(0)' onClick='meetingDateChangeOfAgendaManage(\"".$this->date->toString()."_close\")'>开放</a></span>)
					 <form id='".$this->date->toString()."_close' action='manageagendas.php' method='post'>
						<input type='hidden' name='".ConstantValue::CONST_FIELD_AGENDA_MANAGE_DATE_SELECT."' value='".$this->date->toString()."' />
					 </form>";
		} else {
			return  "(<span id='meetingopen'>开放</span>)";
		}
	}
