...
class MemberManager{
	...
	private $memberTableHeadersText = array("会员编号", "头马编号", "姓名", "会员状态","CC", "CL", "CN Name", "电子邮件",
			"电话号码", "QQ", "Weibo ID", "生日", "P_CC", "P_CL");
	...
	public function getHappyBirthdayContent(){
		echo "目前俱乐部一共有过 ".count($this->allMemberRecords)." 名会员，其中 ".count($this->validMembers)." 名有效会员。<br />";
		if(count($this->happyBirthdayThisMonth) != 0){
			echo "本月（".date("M")."）将有".$this->getNumberOfMembersHaveBirthday()."名会员过生日。生日快乐！！他们是（";
			$firstOne = false;
	...
	public function printActions(){
		...
		echo "<tr><td>";
		echo $this->showAllMembers ? "显示所有会员" : "<a href='javascript:void(0)' onClick='showMembers(\"".$this->containerId."\", \"".$this->managerClubID."\", true)'>显示所有会员</a>";
		echo "&nbsp; &nbsp; &nbsp;";
		echo $this->showAllMembers ? "<a href='javascript:void(0)' onClick='showMembers(\"".$this->containerId."\", \"".$this->managerClubID."\", false)'>显示有效会员</a>" : "显示有效会员";
		echo in_array($this->managerClubID, $this->superadminIds) ? "</td><td id='addMemberAction'><a href='javascript:void(0)' onClick='addANewMember()'>添加新会员</td></tr>" : "</td></tr>";
		echo "</table>";
		echo "</div>";
	}
...
// 		echo "<br />";
	...
	private function printCount($countNum){
		...
		echo "<tr><td>共计：".$countNum."。</td><td id='countBacktoTop'><a href='javascript:void(0)' onClick='setToTop()'>[&nbsp;回到顶部&nbsp;]</a></td></tr>";
		echo "</table>";
		echo "</div>";
	}
